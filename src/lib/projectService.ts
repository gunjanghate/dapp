...
export const projectService = {
  // Purchase a project
  async purchaseProject(projectId: string, buyerAddress: string, price: number, transactionHash?: string): Promise<PurchaseResult> {
    try {
  ...
  async createProject(projectData: ProjectData): Promise<{ success: boolean; project?: any; error?: string }> {
    try {
  ...
  async stakeProject(purchaseId: string, transactionHash?: string, lockPeriodDays: number = 90): Promise<StakeResult> {
    try {
      ...
      const finalVotingPower = Math.floor(baseVotingPower * votingPowerMultiplier);
      ...
      const { data: stake, error: stakeError } = await supabase
        ...
        .insert([{
          ...
          lock_end_date: new Date(Date.now() + lockPeriodDays * 24 * 60 * 60 * 1000).toISOString(),
          status: 'active',
  ...
  async withdrawStake(stakeId: string, transactionHash?: string): Promise<boolean> {
    try {
